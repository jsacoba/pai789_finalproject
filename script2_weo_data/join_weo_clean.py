...
# 2. Read files by calling pd.read_csv function.
...
# 3. For 'gdp_raw' and 'pop_raw', read figures as float in the '"2020"' column.

gdp_raw['2020'] = gdp_raw['2020'].str.replace(',','').astype(float)
pop_raw['2020'] = pop_raw['2020'].str.replace(',','').astype(float)

# 4. Select relevant columns (2020 population and GDP) from 'gdp_raw' and 'pop_raw'.

gdp_clean = gdp_raw.loc[:194,['ISO', '2020']]
pop_clean = pop_raw.loc[:192,['ISO', '2020']]

# 5. Rename 'ISO' columns to 'iso3' from 'gdp_raw' and 'pop_raw'.
...
# 6. Join the 3 data sets above using the 'mapping' file.
