...
# 2. Read the 'wri' files using 'pd.read_csv'.
...
# 5. Map the ISO 3 letter alpha code to each 'wri' file to standardize ID for 
...
# 6. Rename 'WorldRiskIndex' column.
...
# 7. Set the column 'iso3' as index.
...
# 8. Drop the 'Country' column for 2016-2019 data. 
...
# 9. Count number of countries in the 2020 WRI Report.

count_20 = len(wri20)
print('\nNumber of Countries in the 2020 WRI Report:', count_20)

# 10. Join the data from 2020 to 2016 for countries with complete records only for 5 years.

wri16_to_20 = wri20.join(wri19, how='inner').join(wri18, how='inner').join(wri17, how='inner').join(wri16, how='inner')

# 11. Count number of countries with complete records.
...
# 12. Set conditions for risk description numerical scale.
...
# 13. Set descriptive values for each numerical scale.

values = ['Very Low', 'Low', 'Medium', 'High', 'Very High']

# 14. Assign risk description to the 'WRI' of each countires.
...
# 16. Double check for missing values/records.
...
# 18. Create final mapping file.

mapping_wri = wri16_to_20.iloc[:, 0:1] 

# 19. Save cleaned data sets.
